...
#ifdef QT_GUI
const char *walletUrl = "http://www.ailuncoin.com/";//主页显示页面
const char *walletDownloadsUrl = "http://www.ailuncoin.com/downloads/";
const char *forumsUrl = "http://www.ailuncoin.com/";
bool fRestart = false;
...
#else
    const char* pszModule = "arroncoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ArronCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ArronCoin
    // Mac: ~/Library/Application Support/ArronCoin
    // Unix: ~/.ArronCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ArronCoin";
#else
    fs::path pathRet;
...
boost::filesystem::path GetConfigFile()
{
    ...
    std::string conf("arroncoin.conf");
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "arroncoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong arroncoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("arroncoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {

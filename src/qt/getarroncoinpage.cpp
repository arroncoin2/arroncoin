...
GetarroncoinPage::GetarroncoinPage(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::GetarroncoinPage),
    walletModel(0)
{
    ui->setupUi(this);
    ...
    if (fNoHeaders)
        ...
        GUIUtil::header(this, QString(":images/headerGetarroncoinSmall"));
    else
        GUIUtil::header(this, QString(":images/headerGetarroncoin"));
    this->layout()->setContentsMargins(0, HEADER_HEIGHT, 0, 0);

    CookieJar *getarroncoinJar = new CookieJar;
    ui->webView->page()->networkAccessManager()->setCookieJar(getarroncoinJar);
...
GetarroncoinPage::~GetarroncoinPage()
{
    delete ui;
}

void GetarroncoinPage::setModel(WalletModel *model)
{
    this->walletModel = model;

    QUrl url(QString(walletUrl).append("easyarron/"));

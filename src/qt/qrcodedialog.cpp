...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ARC unit input in lnReqAmount, we generate the URI with ARC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::formatMaxDecimals(BitcoinUnits::ARC, ui->lnReqAmount->value(), BitcoinUnits::maxdecimals(BitcoinUnits::ARC)));
            paramCount++;
        }
        else
        {
